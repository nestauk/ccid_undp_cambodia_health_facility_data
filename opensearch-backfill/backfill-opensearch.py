...
def main():
    ...
    parser.add_argument(
        "--lf",
        metavar="LF",
        help="lambda function that posts data to es",
        required=True,
    )
    parser.add_argument(
        "--esarn", metavar="ESARN", help="event source ARN", required=True
    )
    ...
    import_dynamodb_items_to_es(
        args.tn, args.sk, args.ak, args.st, args.rn, args.esarn, args.lf, scan_limit
    )


def import_dynamodb_items_to_es(
    table_name,
    aws_secret,
    aws_access,
    aws_token,
    ...
    while True:
        if not response:
            ...
            response = table.scan(
                ExclusiveStartKey=response["LastEvaluatedKey"], Limit=scan_limit
            )
...
def send_to_eslambda(items, lambda_f):
    global reports
    global partSize
    records_data = {"Records": items}
    records = json.dumps(records_data)
    lambda_response = client.invoke(FunctionName=lambda_f, Payload=records)
    reports = []
    partSize = 0
